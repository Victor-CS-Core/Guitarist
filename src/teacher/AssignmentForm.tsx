...
export function AssignmentForm({ studentId }: { studentId: string }) {
  ...
  const options = activities.filter((a) =>
    student.unlockedLevels.includes(
      skills.find((s) => s.id === a.skillId)!.levelId,
    ),
  );
  ...
  return (
    <form
      ...
      onSubmit={async (e) => {
        ...
        setMessage(
          r.ok
            ? "Practice assigned. Your student can find it on Home."
            : r.error,
        );
      }}
    >
      <h2>Plan the next small step.</h2>
      <p>Choose a focused activity from an unlocked chapter.</p>
      <label>
        Practice activity
        <select
          value={activityId}
